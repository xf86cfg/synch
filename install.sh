...
#     echo_info "${DEFAULT_INSTALL_DIR} exists. Cleaning its content"
#     if sudo rm -r ${DEFAULT_INSTALL_DIR}/*; then
#         echo_success "OK."
#     else
#         echo_error "Failed to clean content of ${DEFAULT_INSTALL_DIR}."
#         exit 1;
#     fi
# else
#     echo_info "${DEFAULT_INSTALL_DIR} doesn't exists."
#     echo_info "Creating ${DEFAULT_INSTALL_DIR}"
#     if sudo mkdir -p "${DEFAULT_INSTALL_DIR}"; then
#         echo_success "OK."
#     else
...
echo_info "\nCopying ${BUILD_NAME} build into ${DEFAULT_INSTALL_DIR}"
if sudo cp $DEFAULT_BUILD_DIR/* ${DEFAULT_INSTALL_DIR}; then
    echo_success "OK."
else
...
echo_info "\nSetting up file permissions on ${DEFAULT_INSTALL_DIR}"
if sudo chown -R $USER:$GROUP $DEFAULT_INSTALL_DIR && chmod +x $DEFAULT_INSTALL_DIR/$EXEC_FILE; then
    echo_success "OK."
else
...
echo_info "\nSetting up file capabilities on ${DEFAULT_INSTALL_DIR}/${EXEC_FILE}"
if sudo setcap cap_net_bind_service=+ep ${DEFAULT_INSTALL_DIR}/${EXEC_FILE}; then
    echo_success "OK."
else
